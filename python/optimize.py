...
def main() -> None:
    ...
    for game_id, game_props in by_game.items():
        try:
            # v1: no external model/sharp/context feeds — The System estimates from line
            ...
            # Translate to the shape routes.ts expects:
            # six_legs: list of leg dicts (SYSTEM_FIRE) or [] (NO_GO / LOCKED)
            # two_demons: always [] here — demon pipeline owns that
            # system: full SystemDecision for frontend consumption
            legs = formatted.get('legs', [])
            output[game_id] = {
                'six_legs':   legs,
                'two_demons': [],          # demon pipeline owns this
                'system':     formatted,   # full decision for frontend
