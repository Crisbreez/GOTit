                  ...
                  return (
                    <div key={leg.id} className={`leg-row${isHit ? ' hit' : isMiss ? ' miss' : ''}`} style={{ opacity: isDnp ? 0.4 : 1 }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        {leg.matchupFitScore != null && (
                          <div data-testid={`text-matchup-fit-${leg.id}`} style={{ fontSize:'0.62rem', fontFamily:'Space Mono,monospace', marginTop:2,
                            color: leg.matchupFitScore > 0 ? 'hsl(142 72% 46%)' : leg.matchupFitScore < 0 ? 'hsl(0 72% 60%)' : 'hsl(var(--muted-foreground))' }}>
                            FIT {leg.matchupFitScore > 0 ? '+' : ''}{(leg.matchupFitScore * 100).toFixed(1)}%
                            {leg.w4 != null && <span style={{ color:'hsl(var(--muted-foreground))' }}> · w4 {leg.w4}</span>}
                          </div>
                        )}
                      </div>
